...
const LogIn = () =>{
    ...
    const handleForgotPassword = async (e) => {
        ...
        // ? Gọi api về authen forgot password 
        const responseForgotPassword  = await authServices.forgotPassword(email);
        if(responseForgotPassword?.status === 200){
            ...
            if(responseForgotPassword?.response?.status === 404){
                toast.update(id,
                    {
                        render: "Email chưa được đăng kí",
                        type: 'error',
                        isLoading: false,
                        autoClose: true,
                        closeButton: 'Close',
    ...
    const handleLogin  = async (e) => {
        ...
        if(loginResponse?.status === 200){
            ...
            if(loginResponse?.response?.status === 400 && loginResponse?.response?.data?.message === 'Password is invalid!'){
                ...
                notify("Email chưa được đăng kí!",'error');
            }else{
